...
export const CATALOG: Record<string, AssetDef> = {
  ...
  USDT: {
    ...
    type: "crypto",
    unit: 0.1,
    upbitMarket: "USDT-KRW",
    title: "PIAREA-Coin-USDT",
    short: "cusdt",
  },
  XRP: {
    symbol: "XRP",
  ...
  USDC: {
    ...
    type: "crypto",
    unit: 0.1,
    upbitMarket: "USDC-KRW",
    title: "PIAREA-Coin-USDC",
    short: "cusdc",
  },
  TRX: {
    symbol: "TRX",
  ...
  HLP: {
    ...
    type: "crypto",
    unit: 0.01,
    upbitMarket: undefined,
    title: "PIAREA-Coin-Hyperliquid",
    short: "chlp",
  },
  BCH: {
    symbol: "BCH",
...
export async function getCryptoUnitKRW(_env: AppEnv, def: AssetDef): Promise<PriceUnitKRW> {
...
export async function getKRXUnitKRW(_env: AppEnv, def: AssetDef): Promise<PriceUnitKRW> {
...
export async function getAssetRowBySymbol(db: any, symbol: string) {
  const r: any = (
    await db.execute(sql`
      SELECT id,symbol,name,kind FROM assets WHERE upper(symbol)=upper(${symbol}) LIMIT 1
    `)
  ).rows?.[0];
  return r || null;
}
export async function holdingUnits(db: any, userId: string, assetId: string) {
  const q = await db.execute(sql`
    ...
    FROM orders WHERE user_id=${userId} AND asset_id=${assetId}
  `);
...
export function registerTradeRoutes(app: Hono<{ Bindings: AppEnv }>) {
  ...
  app.get("/v1/market/price/unit", async c => {
    ...
    if (!def) return err(c, ERR.asset_not_found, "자산을 찾을 수 없어요...", 404);
    let r: PriceUnitKRW;
    ...
    if (!r.ok) return err(c, ERR.price_unavailable, "가격을 가져오지 못했어요...", 503);
    return ok(
      c,
      {
        symbol: def.symbol,
  ...
  app.get("/v1/proxy/upbit", async c => {
    ...
    if (!raw || raw.length > 512) return err(c, ERR.bad_request, "잘못된 요청이에요...", 400);
    let url: URL;
    try {
      url = new URL(raw);
    } catch {
    ...
    const okHost =
      ...
      !url.password &&
      !url.port;
    if (!okHost) return err(c, ERR.bad_request, "허용되지 않는 주소예요...", 400);
    try {
  ...
  app.get("/v1/proxy/yahoo", async c => {
    ...
    if (!allow) return err(c, ERR.bad_request, "허용되지 않는 주소예요...", 400);
    ...
    if (
      ...
      if (sym) {
        ...
        if (Number.isFinite(px) && px > 0) {
          json = {
            quoteResponse: { result: [{ symbol: sym, regularMarketPrice: px }] },
          };
          ok1 = true;
        }
      }
    }
    if (!ok1) return err(c, "yahoo_http", "프록시 요청에 실패했어요...", 502);
  ...
  app.post("/v1/telemetry/ping", async c => {
    try {
      ...
      const u: any = (
        await db.execute(sql`
          SELECT user_id,id,cash_pia,pnl_buffer
          FROM users WHERE id=${dto.id} LIMIT 1
        `)
      ).rows?.[0];
      if (!u) return err(c, ERR.not_found, "유저를 찾을 수 없어요...", 404);
      ...
      return err(c, "telemetry_failed", "상태 갱신에 실패했어요...", (e as any)?.status || 500);
    }
  });
  ...
  app.post("/v1/portfolio/summary", async c => {
    ...
    if (!row) return err(c, ERR.not_found, "유저를 찾을 수 없어요...", 404);
    return ok(
      c,
      {
        id: dto.id,
    ...
    try {
      ...
      for (const r of rows) {
        ...
        if (dto.includePrices) {
          ...
          if (arow) avgBuyPriceKrw = await avgBuyCostKRW(db, user.userId, arow.id);
      ...
      return err(c, "holdings_failed", "보유 자산 조회에 실패했어요...", (e as any)?.status || 500);
    }
  });
  ...
  app.post("/v1/orders/list", async c => {
    try {
      ...
      return err(c, "orders_list_failed", "주문 내역 조회에 실패했어요...", (e as any)?.status || 500);
    }
  });
  ...
  app.post("/v1/orders", async c => {
    try {
      ...
      if (!a) return err(c, ERR.asset_not_found, "자산을 찾을 수 없어요...", 404);
      const def = getAssetDef(dto.symbol);
      if (!def || def.type === "index" || def.type === "indicator") {
        return err(c, ERR.method_not_allowed, "이 자산은 거래할 수 없어요...", 405);
      }
      ...
      if (have < qty - 1e-9) {
        return err(c, ERR.insufficient_quantity, "보유 수량이 부족해요...", 400);
      }
      ...
      return err(c, "order_failed", "주문 처리에 실패했어요...", (e as any)?.status || 500);
    }
  });
  ...
  app.post("/v1/users/signup", async c => {
    try {
      ...
      const existed = (
        await db.execute(sql`
          SELECT user_id FROM users WHERE lower(id)=lower(${dto.id}) LIMIT 1
        `)
      ).rows?.[0];
      if (existed) return err(c, "already_registered", "이미 가입된 계정이에요...", 409);
      ...
      try {
        ...
        if (/duplicate key|unique constraint/i.test(msg)) {
          return err(c, "already_registered", "이미 가입된 계정이에요...", 409);
        }
        throw e;
      }
      return ok(
      ...
      return err(c, "signup_failed", "회원가입 처리 중 오류가 발생했어요...", (e as any)?.status || 500);
    }
  });
  ...
  app.post("/v1/rewards", async c => {
    try {
      ...
      const uAuth = await verifyUserSimple(c.env, dto.id, dto.password).catch(() => {
        throw Object.assign(new Error(ERR.auth_failed), { status: 401 });
      });
      const db = getDbNow(c.env);
      const u0: any = (
        await db.execute(
          sql`SELECT user_id,cash_pia,subscribed FROM users WHERE user_id=${uAuth.userId} LIMIT 1`,
        )
      ).rows?.[0];
      if (!u0) return err(c, ERR.not_found, "유저를 찾을 수 없어요...", 404);
      if (dto.kind === "subscription" && u0.subscribed) {
        return err(c, "already_subscribed", "이미 구독을 신청하셨어요...", 409);
      }
      try {
        await db.execute(sql`
          INSERT INTO rewards(id,user_id,amount_krw,note)
          VALUES(${crypto.randomUUID()},${uAuth.userId},${amount},${dto.note ?? null})
        `);
      } catch (e: any) {
        console.error("rewards insert failed:", e?.message || e);
      }
      const setSub = dto.kind === "subscription" ? sql`, subscribed=TRUE` : sql``;
      const up: any = (
        await db.execute(sql`
          UPDATE users
      ...
      const code =
        (e as any)?.message === ERR.auth_failed ? ERR.auth_failed : "reward_failed";
  ...
  app.get("/v1/users/rankings", async c => {
    try {
      ...
      const limit = Number.isFinite(limitQ) ? Math.min(Math.max(1, limitQ), 100) : 50;
      const db = getDbNow(c.env);
      const rows: any[] = (
        await db.execute(sql`
      ...
      return err(c, "ranking_failed", "유저 순위 조회에 실패했어요...", (e as any)?.status || 500);
    }
  });
}