...
export const users = pgTable("users", {
  ...
  cashPia: numeric("cash_pia", { precision: 18, scale: 2 }).notNull().default("0"),
...
export const userRanks = pgTable("user_ranks", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id").notNull(),
  piaBalance: numeric("pia_balance", { precision: 18, scale: 2 }).notNull().default("0"),
  returnRate: numeric("return_rate", { precision: 9, scale: 4 }).notNull().default("0"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow(),
});